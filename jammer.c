...
static void render_callback(Canvas *const canvas, void *ctx) {
  ...
  if (!plugin_state->is_thread_running) {
    ...
    canvas_set_font(canvas, FontSecondary);
    canvas_draw_str_aligned(canvas, 10, 40, AlignLeft, AlignBottom,
                            "Press Ok button to start");
    if (!nrf24_check_connected(nrf24_HANDLE)) {
      canvas_draw_str_aligned(canvas, 10, 60, AlignLeft, AlignBottom,
                              "Connect NRF24 to GPIO!");
    } else {
      canvas_draw_str_aligned(canvas, 10, 60, AlignLeft, AlignBottom,
                              "You are ready to go!");
    }
    ...
    canvas_draw_str_aligned(canvas, 3, 30, AlignLeft, AlignBottom,
                            "Causing mayhem...");
    canvas_draw_str_aligned(canvas, 3, 40, AlignLeft, AlignBottom,
                            "Please wait!");
    canvas_draw_str_aligned(canvas, 3, 50, AlignLeft, AlignBottom,
                            "Press back to exit.");
  } else {
    canvas_draw_str_aligned(canvas, 3, 10, AlignLeft, AlignBottom,
                            "Unknown Error");
    canvas_draw_str_aligned(canvas, 3, 20, AlignLeft, AlignBottom,
                            "press back");
...
static int32_t mj_worker_thread(void *ctx) {
  ...
  uint8_t
      ping_packet[] = {0xDE, 0xAD, 0xBE, 0xEF, 0xDE, 0xAD, 0xBE, 0xEF, 0xDE,
                       0xAD, 0xBE, 0xEF, 0xDE, 0xAD, 0xBE, 0xEF, 0xDE, 0xAD,
                       0xBE, 0xEF, 0xDE, 0xAD, 0xBE, 0xEF, 0xDE, 0xAD, 0xBE,
                       0xEF, 0xDE, 0xAD, 0xBE, 0xEF}; // 32 bytes, in case we
                                                      // ever need to experiment
                                                      // with bigger packets
...
#define nrf24_TIMEOUT 500
  ...
  uint8_t hopping_channels_1[] = {32, 34, 46, 48, 50, 52, 0,  1,
                                  2,  4,  6,  8,  22, 24, 26, 28,
                                  30, 74, 76, 78, 80, 82, 84, 86};
